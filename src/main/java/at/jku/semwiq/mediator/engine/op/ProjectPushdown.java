...
public class ProjectPushdown extends TransformBase {
	...
	public Op transform(OpOrder opOrder, Op subOp) {
		...
		if (proj != null) {
			...
			// set project only if (projVars < scope vars)
			if (projVars.size() < scopeVars.size()) {
				List<Var> projVarsList = new ArrayList<Var>();
				projVarsList.addAll(projVars); // needs a list
				opOrder.setSubOp(new OpProject(subOp, projVarsList));
			}
			
			// drop orig project if subProjVars == original proj vars
			if (projVars.equals(origProjVars))
				switchChild(proj, opOrder); // drop proj
	...
	private Op transformOp2(OpProject proj, Op2 op2, Op left, Op right) {
		// original project vars
		Set<Var> origProjVars = new HashSet<Var>();
		origProjVars.addAll(proj.getVars()); // projVars as is, we need it later
		...
		// set left/right project only if proj vars < scope vars
		...
		// drop original project if (left U right == original proj vars)
		Set<Var> both = new HashSet<Var>();
		both.addAll(leftProj);
		both.addAll(rightProj);
		if (both.equals(origProjVars))
			switchChild(proj, op2); // drop proj

		return op2;
	}
	
	/* (non-Javadoc)
	 * @see com.hp.hpl.jena.sparql.algebra.TransformBase#transform(com.hp.hpl.jena.sparql.algebra.op.OpJoin, com.hp.hpl.jena.sparql.algebra.Op, com.hp.hpl.jena.sparql.algebra.Op)
	 */
	@Override
	public Op transform(OpJoin opJoin, Op left, Op right) {
		OpProject proj = getProjectParent(opJoin);
		if (proj != null)
			return transformOp2(proj, opJoin, left, right);
		else
			return super.transform(opJoin, left, right);
	}

	/* (non-Javadoc)
	 * @see com.hp.hpl.jena.sparql.algebra.TransformBase#transform(com.hp.hpl.jena.sparql.algebra.op.OpLeftJoin, com.hp.hpl.jena.sparql.algebra.Op, com.hp.hpl.jena.sparql.algebra.Op)
	 */
	@Override
	public Op transform(OpLeftJoin opLeftJoin, Op left, Op right) {
		OpProject proj = getProjectParent(opLeftJoin);
		if (proj != null)
			return transformOp2(proj, opLeftJoin, left, right);
		else
			return super.transform(opLeftJoin, left, right);
	}
	
	/* (non-Javadoc)
	 * @see com.hp.hpl.jena.sparql.algebra.TransformBase#transform(com.hp.hpl.jena.sparql.algebra.op.OpUnion, com.hp.hpl.jena.sparql.algebra.Op, com.hp.hpl.jena.sparql.algebra.Op)
	 */
	@Override
	public Op transform(OpUnion opUnion, Op left, Op right) {
		OpProject proj = getProjectParent(opUnion);
		if (proj != null)
			return transformOp2(proj, opUnion, left, right);
		else
			return super.transform(opUnion, left, right);
	}
	
	/* (non-Javadoc)
	 * @see com.hp.hpl.jena.sparql.algebra.TransformBase#transform(com.hp.hpl.jena.sparql.algebra.op.OpConditional, com.hp.hpl.jena.sparql.algebra.Op, com.hp.hpl.jena.sparql.algebra.Op)
	 */
	@Override
	public Op transform(OpConditional opCond, Op left, Op right) {
		OpProject proj = getProjectParent(opCond);
		if (proj != null)
			return transformOp2(proj, opCond, left, right);
		else
			return super.transform(opCond, left, right);
	}
	
	/* (non-Javadoc)
	 * @see com.hp.hpl.jena.sparql.algebra.TransformBase#transform(com.hp.hpl.jena.sparql.algebra.op.OpDiff, com.hp.hpl.jena.sparql.algebra.Op, com.hp.hpl.jena.sparql.algebra.Op)
	 */
	@Override
	public Op transform(OpDiff opDiff, Op left, Op right) {
		OpProject proj = getProjectParent(opDiff);
		if (proj != null)
			return transformOp2(proj, opDiff, left, right);
		else
