			...
			try {
				...
				while (logBuffer.isActive()) {
					...
					sb.append("<table class=\"log_table\"><tr><td class=\"").append(level).append("\">").append(event.getLevel().levelStr)
						...
						.append("</td> <td class=\"log_msg\">").append(event.getFormattedMessage()).append("<br><span class=\"log_details\">").append(event.getCallerData()[0]).append(" (Thread ").append(event.getThreadName()).append(")</span>").append("</td></tr>");
