...
public abstract class RemoteUpdateWorkerBase extends UpdateWorkerBase {
	 ...
	 */
	public static final int TIMING_GAP = 60000; // ms 
	...
	public void checkAndDownload(String statsUrl, RDFStatsUpdatableModelExt stats, Date lastDownload, boolean onlyIfNewer) throws DataSourceMonitorException {
		...
		if (newer || !onlyIfNewer) {
			...
			if (log.isInfoEnabled())
				log.info("Statistics for " + ds + " are up-to-date" + ((lastDownload != null) ? " (" + lastDownload + ")" : ""));
		}
	}
	...
	private Model retrieveModelData(HttpURLConnection conn, DataSource ds) throws DataSourceMonitorException {
		...
		if (cleanFileUrl.endsWith(".zip")) {
			...
			try {
	            ...
	            try {
	            	...
	            	if (entry == null)
	            		log.error("Failed to extract data from obviously empty zip archive at <" + url + ">.");
	            	else {
	            		log.debug("Loading ZIP file entry '" + entry.getName() + "'...");
	            		String lang = FileUtils.guessLang(cleanFileUrl.substring(0, cleanFileUrl.length()-4)); // original extension (without ".zip")? e.g. .n3
		...
		} else if (cleanFileUrl.endsWith(".tar.gz"))
			throw new DataSourceMonitorException("Decompression of .tar.gz not yet implemented.");
		else if (cleanFileUrl.endsWith(".tar.bz2"))
			throw new DataSourceMonitorException("Decompression of .tar.bz2 not yet implemented.");
		else {
			log.info("Downloading statistics for " + ds + " from <" + url + "> (" + (size / 1024) + "kb)...");
			conn.disconnect(); // abort and use Jena's Model.read(String url)...
			Model model = ModelFactory.createDefaultModel();
			model.read(url);
			return model;
		}
